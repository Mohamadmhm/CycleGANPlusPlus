...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, x, y):
        x_conv = self.relu0(self.normlayer0(self.conv0(self.refpad0(x))))
        x_conv = self.downrelu1(self.downnorm1(self.downconv1(x_conv)))
        x_conv = self.downrelu2(self.downnorm2(self.downconv2(x_conv)))
        x_res = self.res1(x_conv)
        x_res = self.res2(x_res)
        x_res = self.res3(x_res)
        x_res = self.res4(x_res)
        x_res = self.res5(x_res)
        x_res = self.res6(x_res)
        x_res = self.res7(x_res)
        x_res = self.res8(x_res)
        x_res = self.res9(x_res)
        x_deconv = self.uprelu11(self.upnorm11(self.upconv11(x_res)))
        x_deconv = self.uprelu21(self.upnorm21(self.upconv21(x_deconv)))
        x_deconv = self.tanh11(self.conv11(self.refpad11(x_deconv)))

        y_conv = self.relu0(self.normlayer0(self.conv0(self.refpad0(y))))
        y_conv = self.downrelu1(self.downnorm1(self.downconv1(y_conv)))
        y_conv = self.downrelu2(self.downnorm2(self.downconv2(y_conv)))
        y_res = self.res1(y_conv)
        y_res = self.res2(y_res)
        y_res = self.res3(y_res)
        y_res = self.res4(y_res)
        y_res = self.res5(y_res)
        y_res = self.res6(y_res)
        y_res = self.res7(y_res)
        y_res = self.res8(y_res)
        y_res = self.res9(y_res)
        y_deconv = self.uprelu12(self.upnorm12(self.upconv12(y_res)))
        y_deconv = self.uprelu22(self.upnorm22(self.upconv22(y_deconv)))
        y_deconv = self.tanh12(self.conv12(self.refpad12(y_deconv)))

        return x_deconv, y_deconv, self.l2(self.avgx(x_res), self.avgy(y_res))
